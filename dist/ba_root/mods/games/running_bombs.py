...
class RunningBombsGame(bs.TeamGameActivity[Player, Team]):
    ...
    def _kill_player(self):
        try: player = bs.getcollision().opposingnode.getdelegate(PlayerSpaz, True).getplayer(Player, True)
        except: return
        if player.exists():
            player.actor.handlemessage(bs.DieMessage())
            player.actor.shatter()
    def _kill_bomb(self):
        try: bomb = bs.getcollision().opposingnode.getdelegate(Bomb, True)
        except: return
        if bomb.exists():
    ...
    def _drop_bomb_cluster(self) -> None:
        ...
        for _i in range(random.randrange(1, 3)):
            ...
            bs.timer(delay, bs.CallPartial(self._drop_bomb, pos, vel))
            delay += 0.1
