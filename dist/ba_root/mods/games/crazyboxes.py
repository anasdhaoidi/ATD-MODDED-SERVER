...
from bascenev1lib.actor.bomb import Bomb, BombFactory, Blast, ExplodeMessage
    ...
    def __init__(self,
        ...
        self.node = ba.newnode('prop',
                                   ...
                                   attrs={
                                       'position': self.pos,
                                       'velocity': velocity,
                                       'mesh': ba.getmesh('powerup'),
                                       'light_mesh':  ba.getmesh('powerup'),
                                       'body': 'crate',
                                       'mesh_scale': self.data.get('size'),
        ...
        ba.animate(self.node, 'mesh_scale', {0.0:0.0, 0.7:self.data.get('size')})
        ba.timer(1000*0.001, self.start_updating)

    def start_updating(self):
        self.move_timer = ba.Timer(100*0.001, ba.WeakCallPartial(self.update_ai), repeat=True)
    ...
    def explode(self) -> None:
        ...
        if self.node:
            ...
            for callback in self._explode_callbacks:
                callback(self, blast)
        ba.timer(0.001, ba.WeakCallPartial(self.handlemessage, ba.DieMessage()))
