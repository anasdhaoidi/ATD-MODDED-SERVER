...
def slow_motion():
    try:
        ...
        with activity.context:
            gn = activity.globalsnode
            gn.slow_motion = not gn.slow_motion
    except Exception as e:
        print(f'slow_motion error: {e}')
...
def remove_custom_tag(arguments):
    try:
        ...
        for i in session.sessionplayers:
            if i.inputdevice.client_id == int(arguments[0]):
                pdata.remove_tag(i.get_account_id())
    except:
        return
...
def remove_custom_effect(arguments):
    try:
        ...
        for i in session.sessionplayers:
            if i.inputdevice.client_id == int(arguments[0]):
                pdata.remove_effect(i.get_account_id())
    except:
        return
...
def set_custom_effect(arguments):
    try:
        ...
        for i in session.sessionplayers:
            if i.inputdevice.client_id == int(arguments[1]):
                pdata.set_effect(arguments[0], i.get_account_id())
    except:
        return
