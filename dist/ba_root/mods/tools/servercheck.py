...
class checkserver(object):
    def start(self):
        ...
        self.t1 = bs.AppTimer(1, babase.CallPartial(self.check),
                              repeat=True)
    ...
    def check(self):
        ...
        for ros in bs.get_game_roster():
            ...
            try:
                ...
                pass
            # إذا فشل، استخدم account_id كبديل فريد (لا نطرد بسببه)
            if not ip:
                ip = ros.get("account_id", str(ros["client_id"]))
            ...
            if device_id not in deviceClientMap:
                ...
                if len(deviceClientMap[device_id]) >= settings[
                    ...
                    bs.broadcastmessage(
                        ...
                        transient=True, clients=[ros["client_id"]])
...
class LoadProfile(threading.Thread):
    ...
    def run(self):
        ...
        _babase.pushcall(
            CallPartial(on_player_join_server, self.pbid, player_data, self.ip,
                 self.device_id),
...
def save_age(age, pb_id, display_string):
    _babase.pushcall(babase.CallPartial(pdata.add_profile, pb_id, display_string,
    ...
    if get_account_age(age) < settings["minAgeToJoinInHours"]:
        ...
        _babase.pushcall(babase.CallPartial(kick_by_pb_id, pb_id, msg),
                         from_other_thread=True)
