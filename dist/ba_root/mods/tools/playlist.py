...
def withDelay(session, playlist):
    ...
    _babase.pushcall(babase.CallPartial(updateSession, session, playlist),
...
def set_next_map(session, game_map):
    ...
    next_activity = playlist(session)
    if next_activity and hasattr(next_activity, 'session'):
        session.setactivity(next_activity)
