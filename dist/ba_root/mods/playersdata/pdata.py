    ...
    if CacheData.profiles == {}:
        try:
            ...
            try:
                ...
                f.close()
                profiles = json.loads(content_b) if content_b else {}
                print("Falling back to profiles.json.backup")
            except Exception:
                profiles = {}
                print("Both profiles files failed, starting fresh.")
            # إصلاح profiles.json الفارغ
            try:
                with open(PLAYERS_DATA_PATH + "profiles.json", "w") as fw:
                    json.dump(profiles, fw)
            except Exception:
                pass
            CacheData.profiles = profiles
            return profiles
...
def add_profile(
    ...
    for ros in bs.get_game_roster():
        if ros['account_id'] == account_id:
            cid = ros['client_id']
    try:
        ip = _bascenev1.get_client_public_device_uuid(cid) and _bascenev1.get_client_ping(cid)
    except Exception:
        ip = None
    try:
        roster = bs.get_game_roster()
        ip = next((r.get("players",[{}])[0].get("spec_string","") for r in roster if r["client_id"] == cid), None)
    except Exception:
        pass
...
def get_custom() -> dict:
    ...
    if CacheData.custom == {}:
        ...
        custom.setdefault("boughtcommands", {})

    return CacheData.custom


def get_coins(account_id: str) -> int:
    """يُرجع رصيد العملات للاعع من الـ cache المباشر."""
    # نضمن تحميل الـ cache أولاً
    get_custom()
    return CacheData.custom.get("coins", {}).get(account_id, 0)


def add_coins(account_id: str, amount: int) -> None:
    """يضيف أو يخصم عملات للاعع — يُحدّث الـ cache فقط بدون حفظ فوري."""
    custom = get_custom()
    custom.setdefault("coins", {})
    custom["coins"].setdefault(account_id, 0)
    custom["coins"][account_id] += amount
    # لا نحفظ هنا — الحفظ يتم في dump_cache الدورية
